...
export class ConstellationYou {
  ...
  onStarClick(event) {
    ...
    if (intersects.length > 0) {
      ...
      if (clickedStar.userData.selected) {
        console.log('⚠️ Star already selected');
        return;
      }
      ...
      console.log('✅ Star selected. Total selected:', this.selectedStars.length);
      ...
      if (this.selectedStars.length > 1) {
        ...
        console.log('🔗 Drawing line! From star', this.selectedStars.length - 2, 'to star', this.selectedStars.length - 1);
        this.drawLineBetweenStars(previousStar, clickedStar);
      } else {
        console.log('ℹ️ First star selected, no line to draw yet');
      }

      // Check for constellation match
      this.checkConstellationMatch();
    } else {
      console.log('❌ No star intersection detected');
    }
  }
  ...
  drawLineBetweenStars(star1, star2) {
    console.log('📏 drawLineBetweenStars called');
    console.log('  Star 1 position:', star1.position.x, star1.position.y, star1.position.z);
    console.log('  Star 2 position:', star2.position.x, star2.position.y, star2.position.z);
    ...
    // Create a VERY thick, glowing cylinder that's impossible to miss
    const geometry = new THREE.CylinderGeometry(0.15, 0.15, distance, 16);
    const material = new THREE.MeshBasicMaterial({
      color: 0xffff00,  // Bright yellow
      transparent: true,
      opacity: 0.9,  // Start very visible
      depthTest: false,
      depthWrite: false,
    ...
    line.position.copy(star1.position).add(star2.position).divideScalar(2);

    // Rotate to align with the two stars
    ...
    console.log('✨ Line mesh created and added to scene. Total lines:', this.lines.length);
    console.log('  Line position:', line.position.x, line.position.y, line.position.z);
    console.log('  Line opacity:', material.opacity);
    ...
    gsap.to(material, {
      ...
      repeat: 1,
      ease: 'power2.inOut',
      onStart: () => console.log('🎬 Line pulse animation started'),
      onComplete: () => {
        console.log('✅ Line pulse animation complete');
